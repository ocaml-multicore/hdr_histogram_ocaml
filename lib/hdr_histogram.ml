...
module Stdio = struct
  ...
  let fopen name mode =
    ...
    if is_null file then
      raise_unix_error errno "fopen" name;
    file

  let fclose file =
    let rc, errno = C.Stdio.fclose file in
    if rc <> 0 then
      raise_unix_error errno "fclose" ""
...
let hdr_percentiles_print hdr filename ticks_per_half_distance value_scale format =
  Stdio.with_file filename "w" @@ fun file ->
  let errno = C.Function.hdr_percentiles_print hdr file ticks_per_half_distance value_scale format in
  (* returns EIO on error *)
  if errno <> 0 then
    Stdio.raise_unix_error (Signed.SInt.of_int errno) "hdr_percentiles_print" filename
